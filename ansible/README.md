...
- **observability.yml** — Prometheus, Grafana, node_exporter, cAdvisor, Dozzle, `ctop`, and **`lnav`** on the host (`lnav-docker-all` opens all container json-file logs; `lnav-journal` pipes `journalctl`). Caps **journald** size/retention and deploys **`/etc/docker/daemon.json`** json-file **log rotation** (`max-size` / `max-file`) when Docker is present. Grafana at `{{ grafana_fqdn }}`, Dozzle at `{{ dozzle_fqdn }}` behind Traefik.
...
- **mail-flow-lab.yml** — lab-only: SMTP submission (587 STARTTLS + AUTH) and IMAP fetch (993) against **`mail_ip`**, so you can validate the mail loop while public MX still points elsewhere. Requires `mail_flow_lab_test_user` / `mail_flow_lab_test_password` in `group_vars/all.yml`. Defaults **`mail_flow_lab_tls_insecure: true`** (skip cert verify — typical for Origin/snake-oil); set **`false`** for strict verification against public CAs.
